...
- `system`: runtime/environment checks (manual opt-in).
- `uat`: user-facing acceptance scenarios (documented manual tests).

## Automated Regression Policy
- Default command: `pytest`
- Default run includes: unit + component + integration.
- Default run excludes: `manual`, `system`, and `slow`.
...
- Include slow automated checks: `pytest -m "not manual and not system"`
